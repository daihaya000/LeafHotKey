...
- 保護の作動開始・終了はトレイのバルーンで知らせる（退避できなかった場合も知らせる）。
...
| トレイの「再起動」でアプリが戻らない | `%LOCALAPPDATA%\LeafHotKey\restart.log` を見る（停止・再ビルド・起動の判定と失敗理由が残る）。起動バッチはロックで直列化されるため同時実行では1つが待機し、実行が中断されてロックが残った場合は3分後に自動で解消する |
