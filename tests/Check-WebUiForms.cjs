...
for (const id of ['ime-disable', 'send-delay', 'default-blind', 'profile-dialog', 'rule-list']) {
  assert.ok(html.includes(`id="${id}"`), `Missing form control: ${id}`);
}
for (const handler of ['openProfileEditor', 'applyProfileEditor', 'syncSettingsFromForms']) {
